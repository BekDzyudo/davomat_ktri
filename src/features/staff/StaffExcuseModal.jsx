...
// `features/attendance/ExcuseModal.jsx`ning xodim uchun ekvivalenti —
// talaba/xodim uchun "sababli" belgilash oynasi shaklan bir xil, lekin
// har biri o'z API'siga ulanadi (`submitStaffExcuse`), shu sabab alohida
// (talabaning `ExcuseModal`iga tegib, uni buzish xavfisiz) komponent.
export default function StaffExcuseModal({ member, initialReason, onClose, onConfirm, isSaving = false }) {
  ...
  const handleSubmit = (event) => {
    ...
    onConfirm({ reason: reason.trim(), file })
  }
    ...
    <Modal open onClose={onClose} title="Sababli deb belgilash" size="sm">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
        <p className="text-sm text-base-content/60">
          <span className="font-medium text-base-content">{member.name}</span> uchun bugungi kelmaslik
          sababini kiriting
        </p>
